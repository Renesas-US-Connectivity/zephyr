...
static int erpc_wifi_socket_addr_to_posix(const struct sockaddr *addr,
				struct ra_erpc_sockaddr *addr_erpc_wifi)
{
	int err;

	err = erpc_wifi_socket_family_to_posix(addr_erpc_wifi->sa_family, &addr->sa_family);
	if (err) {
...
{
	LOG_DBG("erpc_wifi_socket_ioctl");
...
{
	LOG_DBG("erpc_wifi_socket_shutdown");
		       ...
		       socklen_t addrlen)
{
	...
	LOG_DBG("erpc_wifi_socket_connect");
...
static int erpc_wifi_socket_listen(void *obj, int backlog)
{
	...
	LOG_DBG("erpc_wifi_socket_listen");
	LOG_DBG("fd: %d", sock->fd);
...
static int erpc_wifi_socket_accept(void *obj, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	LOG_DBG("erpc_wifi_socket_accept");
	LOG_DBG("fd: %d", sock->fd);
			  ...
			  const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	LOG_DBG("erpc_wifi_socket_sendto");
	LOG_DBG("len: %d", len);
	LOG_DBG("fd: %d", sock->fd);
			    ...
			    struct sockaddr *src_addr, socklen_t *addrlen)
{
	...
	LOG_DBG("erpc_wifi_socket_recvfrom");
	LOG_DBG("fd: %d", sock->fd);
	...
	//ret = ra6w1_recvfrom(sock->fd, buf, max_len, flags, &addr_erpc_wifi, &addr_len);
	...
	//ret = erpc_wifi_socket_addr_to_posix(src_addr, &addr_erpc_wifi);
...
{
	LOG_DBG("erpc_wifi_socket_getsockopt");
...
{
	LOG_DBG("erpc_wifi_socket_setsockopt");
