...
static void erpc_wifi_server_thread(void *arg1, void *unused1, void *unused2) 
{
	...
	while (1) {
		...
		/* TODO - This delay is currently required when using SPI transport
		   because the poll routine returns if no data is pending and so
		   this thread is constantly running, preventing logging from
		   working...*/
		k_sleep(K_MSEC(10));
		//k_yield();
	}
