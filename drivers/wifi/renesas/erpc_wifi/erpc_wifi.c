			     ...
			     struct wifi_iface_status *status)
{
	...
	LOG_DBG("erpc_wifi_mgmt_iface_status");
...
void ra_erpc_server_event_handler(const ra_erp_server_event_t * event)
{
	...
	LOG_DBG("erpc_wifi_server_event_handler - event_id: %d", event->event_id);

	switch (event->event_id) {
		case eDeviceReset:
		{
			erpc_wifi_driver_data.if_ready = true;
			break;
		}
		case eNetworkInterfaceIPAssigned:
		{
			...
			net_if_ipv4_addr_add(iface, &ip_addr, NET_ADDR_DHCP, 0);
			net_if_ipv4_set_gw(iface, &gw_addr);
			net_if_ipv4_set_netmask_by_addr(iface, &ip_addr, &netmask);
			break;
		}
		default:
			break;
...
static int erpc_wifi_init(const struct device *dev)
{
	...
	#else
		// TODO - replace with semaphore or add a timeout...
		while (!data->if_ready) {
			k_sleep(K_MSEC(10));
		}
	#endif
